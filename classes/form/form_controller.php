...
/**
 * Form controller for local_adele.
 *
 * @package     local_adele
...
namespace local_adele\form;

use \local_adele\adele;
...
abstract class form_controller {
     ...
     */
    public function __construct(adele $adele, array $formdata, \stdClass $moreargs) {
        $this->adele = $adele;
    ...
    protected function construct_mform() {
        $formclass = '\\local_adele\\form\\' . static::$formname;
     ...
     */
    public static function get_controller(string $formname, adele $adele, array $formdata, \stdClass $moreargs) {

        switch ($formname) {
            case 'learninggoal_edit':
                return new learninggoal_edit_controller($adele, $formdata, $moreargs);

            default:
                throw new \moodle_exception('unknown_form', 'adele');
        }
    }
}