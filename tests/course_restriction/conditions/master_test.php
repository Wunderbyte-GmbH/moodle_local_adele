...
use local_adele\course_restriction\conditions\master;
...
final class master_test extends advanced_testcase {
     ...
     * @covers \local_adele\course_restriction\conditions\master
     */
    ...
    /**
     * Test get_restriction_status method.
     * @covers \local_adele\course_restriction\conditions\master
     */
    public function test_get_restriction_status(): void {
        ...
        $node = [
            'data' => [
                'completion' => [
                    'master' => ['restriction' => true],
                ],
            ],
        ];
        $userid = 1;
        $status = $instance->get_restriction_status($node, $userid);

        $this->assertTrue($status);

        $node['data']['completion']['master']['restriction'] = false;
        $status = $instance->get_restriction_status($node, $userid);
